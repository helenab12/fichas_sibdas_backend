...
function atualizarIndicadorIMC() {
  if (!pesoInput || !alturaInput || !indicadorIMC) return;
  const peso = parseFloat(pesoInput.value);
  const altura = parseFloat(alturaInput.value);
  if (isNaN(peso) || isNaN(altura) || altura === 0) {
    indicadorIMC.textContent = "Os resultados aparecerão aqui após clicar em Calcular.";
    indicadorIMC.className = "text-muted";
    return;
  }
  const imc = calcularIMC(peso, altura);
  const resultadoIMC = verificarIMC(imc);
  indicadorIMC.textContent = `${resultadoIMC.texto} (IMC: ${imc.toFixed(2)})`;
  indicadorIMC.className = resultadoIMC.cor;
}

if (pesoInput && alturaInput && indicadorIMC) {
...
function avaliarCondicoes() {
  ...
  if (gravida) {
    mensagemElement.textContent =
      "Encaminhar para o Professor João Faria (Especialista em exercício para grávidas).";
  } else if (problemasCostas && diabetico) {
    mensagemElement.textContent =
      "Necessário plano adaptado. Agendar consulta com especialista.";
  } else if (problemasCostas || diabetico) {
    mensagemElement.textContent =
      "Recomenda-se avaliação personalizada antes de iniciar o treino.";
  } else {
    mensagemElement.textContent =
      "Sem restrições. Pode iniciar o plano de treino geral.";
  }
}
